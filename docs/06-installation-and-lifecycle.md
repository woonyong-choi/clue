...
```bash
brew install woonyong-kr/tap/kyro
```
...
```text
kyro_Darwin_arm64.tar.gz
kyro_Darwin_x86_64.tar.gz
kyro_Linux_arm64.tar.gz
kyro_Linux_x86_64.tar.gz
checksums.txt
checksums.txt.sig
sbom.spdx.json
```

CLI는 `kyro` 바이너리 하나로 설치한다. AWS Kiro의 `kiro`·`kiro-cli`와 파일명이 다르므로 기술적인 실행 파일 충돌은 없다.
...
```bash
kyro install --database external --database-url-secret kyro-db
```
...
1. 로컬 profile 확인
2. 현재 kubeconfig context에서 Kyro installation identity 탐색
3. 저장된 Hub endpoint와 인증서 fingerprint 검증
4. 인증 토큰이 없거나 만료되었으면 다시 로그인

```bash
kyro context list
kyro context use production
kyro status
```
...
`kyro install`은 cluster-scoped installation identity를 사전 확인한다. 활성 Hub가 발견되면 기본적으로 설치를 거부하고 기존 설치 정보를 보여준다.

```text
Kyro Hub already exists
Installation: kyro-prod-01
Namespace: kyro-system
Endpoint: https://kyro.example.com

Use `kyro login` or `kyro hub migrate`.
```
...
```bash
kyro fleet join --context staging --hub prod-hub
```

Agent는 한 Hub에만 active enrollment를 가진다. 다른 Hub에 연결하려면 다음 절차가 필요하다.

```bash
kyro agent transfer --cluster staging --to new-hub
```
...
```bash
kyro hub migrate plan --to-context new-management
kyro hub migrate execute --backup-location s3://...
kyro hub migrate verify
```
