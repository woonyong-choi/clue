...
이 문서 세트는 지금까지 논의한 제품 방향과 기존 `k8s-clue-python-reference`에서 검증한 내용을 Clue의 새 구현 계획으로 정리한 것이다.

## 한 문장 정의

> Clue는 Kubernetes 장애 증거를 수집하고, 판단 근거와 함께 원인을 설명하고, 안전한 수정안을 제안한 뒤 실제 회복까지 검증하는 도구다.
...
| [11. 기존 k8s-clue-python-reference 이전](11-current-k8s-clue-python-reference-migration.md) | 보존할 계약, 버릴 복잡성, 수직 이전 전략 |
| [12. Java와 Python 상세 비교](12-java-vs-python.md) | 접근 제어, interface, 타입, 동시성, 배포와 Clue 적용 비교 |
| [13. Python 선행·Java 포팅 계획](13-python-first-java-port-plan.md) | k8s-clue-python-reference 정리 완료 조건과 Java 단계별 인수 계획 |
| [14. Python 상세 정리·Java 인수 연결](14-python-refactor-handoff-map.md) | k8s-clue-python-reference 작업 ID별 산출물과 Java module 시작·종료 조건 |

## 확정한 명칭

| 개념 | 명칭 |
|---|---|
| 제품과 CLI 명령 | Clue / `clue` |
| 중앙 제어 서버 | Clue Hub |
| 클러스터 수집 프로세스 | Clue Agent |
| 웹 UI | Clue Console |
| PostgreSQL 데이터 계층 | Clue Store |
...
- `k8s-clue-python-reference`: Python 기반 행동 참조 구현으로 먼저 정리한다.
- `Clue`: Java 기반 제품 구현을 시작할 새 디렉터리다.
- 이 문서 작성 시점에는 Clue 애플리케이션 코드가 없다.
